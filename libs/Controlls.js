class Controlls {
  ...
  onSlide(event) {
    this.at = Number(this.slider.value) / (this.duration + 1)
    this.animate(this.at)
  }
  ...
  slideBy(value) {
    let at = Number(this.slider.value) + (this.slider.max * value)
    if (at >= this.slider.max) {
      at = this.slider.max - 1
    } else if (at < 0) {
      at = 0
    }
    this.slider.value = at
    this.onSlide()
  }
